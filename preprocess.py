...
def save_mix_drug_cell_matrix():
    ...
    xd,xds, xc, y = np.asarray(xd),np.asarray(XDS), np.asarray(xc), np.asarray(y)
    ...
    train_data = TestbedDataset(root='data', dataset=dataset+'_train_mix', xd=xd_train,xds=xds_train, xt=xc_train, y=y_train, smile_graph=smile_graph)
    val_data = TestbedDataset(root='data', dataset=dataset+'_val_mix', xd=xd_val,xds=xds_val, xt=xc_val, y=y_val, smile_graph=smile_graph)
    test_data = TestbedDataset(root='data', dataset=dataset+'_test_mix', xd=xd_test,xds=xds_test, xt=xc_test, y=y_test, smile_graph=smile_graph)
...
def save_blind_drug_matrix():
    ...
    xd_train,xds_train, xc_train, y_train = np.asarray(xd_train),np.asarray(xds_train), np.asarray(xc_train), np.asarray(y_train)
    xd_val,xds_val, xc_val, y_val = np.asarray(xd_val),np.asarray(xds_val), np.asarray(xc_val), np.asarray(y_val)
    xd_test,xds_test, xc_test, y_test = np.asarray(xd_test),np.asarray(xds_test), np.asarray(xc_test), np.asarray(y_test)

    dataset = 'GDSC'
    print('preparing ', dataset + '_train.pt in pytorch format!')
    train_data = TestbedDataset(root='data', dataset=dataset+'_train_blind', xd=xd_train,xds=xds_train, xt=xc_train, y=y_train, smile_graph=smile_graph)
    val_data = TestbedDataset(root='data', dataset=dataset+'_val_blind', xd=xd_val,xds=xds_val, xt=xc_val, y=y_val, smile_graph=smile_graph)
    test_data = TestbedDataset(root='data', dataset=dataset+'_test_blind', xd=xd_test,xds=xds_test, xt=xc_test, y=y_test, smile_graph=smile_graph)
...
def save_blind_cell_matrix():
    ...
    dataset = 'GDSC'
    print('preparing ', dataset + '_train.pt in pytorch format!')
    train_data = TestbedDataset(root='data', dataset=dataset+'_train_cell_blind', xd=xd_train,xds=xds_train, xt=xc_train, y=y_train, smile_graph=smile_graph)
    val_data = TestbedDataset(root='data', dataset=dataset+'_val_cell_blind', xd=xd_val,xds=xds_val, xt=xc_val, y=y_val, smile_graph=smile_graph)
    test_data = TestbedDataset(root='data', dataset=dataset+'_test_cell_blind', xd=xd_test,xds=xds_test, xt=xc_test, y=y_test, smile_graph=smile_graph)
